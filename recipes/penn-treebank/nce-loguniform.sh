...
NOISE_DISTRIBUTION=log-uniform
NOISE_DAMPENING=0.75
LEARNING_RATE=20

rm -f "${OUTPUT_DIR}"/nnlm.*
mv -f "${script_dir}/nce-loguniform.log" "${script_dir}/nce-loguniform.log~" 2>/dev/null || true
train | tee "${script_dir}/nce-loguniform.log"
compute_perplexity | tee --append "${script_dir}/nce-loguniform.log"
