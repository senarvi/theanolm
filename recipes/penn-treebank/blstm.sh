...
VALIDATION_FREQ=1
PATIENCE=0
ARCHITECTURE_FILE="${arch_dir}/word-blstm256.arch"
COST=cross-entropy
LEARNING_RATE=1
#DEBUG=1

rm -f "${OUTPUT_DIR}/nnlm.h5"
mv -f "${script_dir}/blstm.log" "${script_dir}/blstm.log~" 2>/dev/null || true
train | tee "${script_dir}/blstm.log"
compute_perplexity | tee --append "${script_dir}/blstm.log"
